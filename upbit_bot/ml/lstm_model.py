...
class LSTMConfig:
    ...
    checkpoint_name: str = "lstm_best.pt"
    max_epochs: int = 50
...
#
# class LSTMTrainer:
#     """LSTM 학습 + Incremental 업데이트 관리.
#
#     사용법 (Google Colab):
#         config = LSTMConfig(max_epochs=50)
...
#         self._cfg = config or LSTMConfig()
#         self._device = torch.device(self._cfg.device)
#         self._model: LSTMBinaryClassifier | None = None
#         self._optimizer: torch.optim.Adam | None = None
#         self._criterion = nn.BCELoss(reduction="none")  # 가중치 적용을 위해 none
#
#     def _build_model(self) -> LSTMBinaryClassifier:
#         model = LSTMBinaryClassifier(self._cfg)
...
#
#         ds_train = LSTMDataset(X_train, y_train, w_train)
#         ds_val   = LSTMDataset(X_val, y_val)
...
#                 loss_per = self._criterion(pred, y_b)
#                 loss = (loss_per.squeeze() * w_b).mean()
#                 loss.backward()
...
#
#         Args:
#             X_new: (n, seq_len, n_features)
#             y_new: (n,)
#             weight_multiplier: 새 샘플 가중치 (기본 ×2.0)
#
#         Returns:
#             업데이트 손실값
#         """
...
#         ds = LSTMDataset(X_new, y_new, weights)
...
class LSTMTrainer:
    """LSTMTrainer 스텁 — Phase B 비활성 시 임포트 안전 보장.
    ...
    def train(self, *args: Any, **kwargs: Any) -> dict[str, list[float]]:
        ...
        raise NotImplementedError(
            "LSTMTrainer.train()은 Phase B 이후 활성화됩니다. "
    ...
    def predict(self, *args: Any, **kwargs: Any) -> Any:
        """[Phase B 비활성] 호출 시 NotImplementedError."""
        raise NotImplementedError(
            "LSTMTrainer.predict()은 Phase B 이후 활성화됩니다."
        )

    def incremental_update(self, *args: Any, **kwargs: Any) -> float:
        """[Phase B 비활성] 호출 시 NotImplementedError."""
        raise NotImplementedError(
            "LSTMTrainer.incremental_update()은 Phase B 이후 활성화됩니다."
        )
    ...
    def load(self, path: str | Path | None = None) -> None:
        ...
        raise RuntimeError(
            "LSTMTrainer.load()은 Phase B 이후 활성화됩니다. "
